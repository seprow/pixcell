...
class MultiClassMaskBuilder(BaseAnnotationBuilder):
    ...
    def build(
        ...
        segmentation = SegmentationData.from_annotation(
            annotation
        )

        return SliceAnnotation(
            annotation=segmentation.multiclass_mask,
            slice_id=(
                source.parent.name,
                source.stem,
            ),
...
class MultiClassMaskVolumeBuilder(
